...
public abstract class HttpHeaderFactory {
    ...
    public String getWeiboAuthHeader(String method, String url, Parameters params){
    	try {
    		final long timestamp = System.currentTimeMillis() / 1000;
            final long nonce = timestamp + (new Random()).nextInt();
            // step 2: authParams�������ô���1.���ܴ�һ���� 2.������Authorizationͷ��
            Parameters authParams = this.generateAuthParameters(nonce, timestamp);
            // ������ڼ���signature�ģ�����
            Parameters signatureParams = this.generateSignatureParameters(authParams, params, url);
    ...
    private String generateAuthSignature(final String method, Parameters signatureParams,
            final String url) {
    ...
    private Parameters generateSignatureParameters(Parameters authParams,
    		Parameters params, String url){
    	Parameters signatureParams = new Parameters();
        signatureParams.addAll(authParams);
        signatureParams.addAll(params);
        this.parseUrlParameters(url, signatureParams);
        Parameters lsp = generateSignatureList(signatureParams);
        return lsp;
    }

    private Parameters generateAuthParameters(long nonce, long timestamp){
    ...
    public abstract Parameters generateSignatureList(Parameters bundle);

    // add additional parameters to des key-value pairs,support to expanding
    // params
    public abstract void addAdditionalParams(Parameters des, Parameters src);

    // ����url�в����,�洢��signatureBaseParams
    public void parseUrlParameters(String url, Parameters signatureBaseParams){
    ...
    public static String encodeParameters(Parameters postParams, String splitter, boolean quot) {
