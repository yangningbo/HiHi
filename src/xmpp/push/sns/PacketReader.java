...
import xmpp.push.sns.Connection.ListenerWrapper;
...
import xmpp.push.sns.packet.ReceiptIQ;
import xmpp.push.sns.packet.XMPPError;
import xmpp.push.sns.sasl.SASLMechanism.Challenge;
import xmpp.push.sns.sasl.SASLMechanism.Failure;
import xmpp.push.sns.sasl.SASLMechanism.Success;
...
class PacketReader {
    ...
    protected void init() {
        ...
        readerThread.setName("Liaos Packet Reader (" + connection.connectionCounterValue + ")");
        ...
        listenerExecutor = Executors.newSingleThreadExecutor(new ThreadFactory() {
			...
			public Thread newThread(Runnable runnable) {
                Thread thread = new Thread(runnable,
                        "Liaos Listener Processor (" + connection.connectionCounterValue + ")");
            ...
            do {
                if (eventType == XmlPullParser.START_TAG) {
                    if (parser.getName().equals("message")) {
                    	...
                    	Log.e("PacketReaderThread", packet.getPacketID()+"\n"+packet.toXML());
                    	if(!TextUtils.isEmpty(packet.getPacketID()) && !TextUtils.isEmpty(message.getBody())){
                    		ReceiptIQ iq = new ReceiptIQ();
